...
<Steps>
  <Step title="Pick provider slug">
    Select a valid provider from [Supported Providers](/guides/introductions/supported-providers).
  </Step>
...
1. [Webhooks](/guides/collections/webhook) for async payment updates.
2. `GET /payments/v1/payments/verify/{reference}` for backend verification.

## Related Docs

<CardGroup cols={3}>
  <Card title="Direct Charge Flows" icon="mobile-screen-button" href="/guides/collections/direct-charge/mobile-money">
    Understand `processing`, `redirect`, and pre-OTP patterns.
  </Card>
  <Card title="Example Payloads" icon="code" href="/guides/collections/direct-charge/examples">
